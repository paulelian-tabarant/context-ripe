...
`buildApp(db, opts)` in `src/app.ts` wires all routes together and initializes the schema.
...
- Schema is initialized in `initializeSchema(db)` at app startup (`CREATE TABLE IF NOT EXISTS`).
